...
int main()
{
    ...
    for( x = 0; x < hash_bucket_row; x++)
    {
        for( y = 0; y < hash_bucket_column; y++)
        {
...
int c,r=0;
        for( r = 0; r < image_square; r++) {
            for( c = 0; c < image_square; c++) {
                if(temp != NULL)
                {
        ...
        if(ans == '2')
        {
            ...
            for( tn = 0; tn < temp_name_length - 1; tn++) { temp_name[tn] = user_data.saved_username[tn];}
